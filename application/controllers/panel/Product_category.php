...
class Product_category extends CO_Panel_Controller
{
    ...
    public function __construct()
    {
        ...
        $controller_config['disable_product_category_seo'] = true;
    ...
    public function all()
    {
        $parent_categories = $this->product_category_model->get_all_for_panel(TRUE);
    ...
    public function add($lang = 1)
    {
        ...
        $this->form_validation->set_rules('categorySlugTitle', 'categorySlugTitle', 'trim|required|is_unique[product_category.title_slug]');
    ...
    {
        if ($id > 0 && $lang == '1') {
            $parent_category = $this->product_category_model->get_for_panel($id);
    ...
    public function delete_category($id){
        ...
        $product=$this->product_model->get_product_by_category($id);
        $go_forward=null;
        if($product){
            $this->session->set_flashdata('error', 'Products are still in this category. You must first eliminate all products before you can delete it.');
            redirect('panel/product_category/all', 'refresh');
        }else{
            if($this->product_category_model->delete_category($id)){
                $go_forward=true;
            }else{
                $go_forward=false;
            }
