...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author    Mathew
	 */
	public function login($identity, $password = '', $remember = FALSE, $is_social_auth = FALSE)
	{
		$this->trigger_events('pre_login');
		if ($is_social_auth !== TRUE) {
		...
		if ($query->num_rows() === 1) {
			...
			if ($this->verify_password($password, $user->password, $identity) || $is_social_auth === TRUE) {
				if ($user->active == 0) {
					$this->trigger_events('post_login_unsuccessful');
