...
class Partner_model extends CO_Core_Model {
    ...
    public function get_all($filter = array(), $limit = "") {
        ...
        $order_by = '';
        $where = "active = '1'";
        if (!empty($filter['partner_name'])) {
            $where .= " and partner_name LIKE '%" . $filter['partner_name'] . "%'";
        }
    ...
    public function get_by_lang($id = '', $lang = '1', $fetch_row = TRUE) {
        ...
        $case_columns = ", CASE WHEN lp.language='$lang' AND  lp.partner_name!='' THEN lp.partner_name ELSE p.partner_name END AS partner_name";
    ...
    public function get_languages($id, $active = TRUE) {
        $query = "select group_concat(DISTINCT language) as languages from partner where language_parent='$id' OR id='$id'";
        if ($active == TRUE) {
            $query .= " AND active='1'";
        }
