...
class Solution_file_description_model extends CO_Core_Model {

    protected $tableName = 'solution_file_description';
    ...
    public function delete_file_description($file_id, $lang = 1) {
        $query = "delete from solution_file_description where file_id='$file_id' AND language='$lang'";
        return $this->full_query($query);
    }

    public function get_languages($id, $active = TRUE) {
        $query = "select group_concat(DISTINCT language) as languages from solution_file_description where file_id='$id'";
        if ($active == TRUE) {
            $query .= " AND active='1'";
        }
