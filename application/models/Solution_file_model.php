...
class Solution_file_model extends CO_Core_Model {

    protected $tableName = 'solution_files';
    ...
    public function get_solution_files($id, $active = FALSE) {
        $where = "id='$id'";
        if ($active == TRUE) {
            $where .= " AND active='1'";
        }
    ...
    public function get_file($filter = array()) {
        $query = "select f.*,fd.id as description_id, fd.title as title, fd.description as description from solution_files as f 
		left join solution_file_description as fd on fd.file_id= f.id where f.id>0";
        if (!empty($filter['file_id'])) {
            $query .= " AND f.id='" . $filter['file_id'] . "'";
        }
        if (!empty($filter['solution_id'])) {
            $query .= " AND f.solution_id='" . $filter['solution_id'] . "'";
        }
    ...
    public function delete_file($file_id) {
        if ($file_id > 0) {
            $query = "delete from solution_files where id='$file_id'";
            return $this->full_query($query);
        }
    }
