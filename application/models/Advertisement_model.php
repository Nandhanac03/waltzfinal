...
class Advertisement_model extends CO_Core_Model {
    ...
    public function get_all($filter = array(), $limit = "") {
        ...
        $order_by = '';
        $where = "active = '1'";
        if (!empty($filter['advertisement_title'])) {
            $where .= " and advertisement_title LIKE '%" . $filter['advertisement_title'] . "%'";
        }
    ...
    public function get_languages($id, $active = TRUE) {
        $query = "select group_concat(DISTINCT language) as languages from advertisement where language_parent='$id' OR id='$id'";
        if ($active == TRUE) {
            $query .= " AND active='1'";
        }
