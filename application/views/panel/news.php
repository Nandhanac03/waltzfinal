                                    ...
                                    if ($all_news): $i = 0;
                                        foreach ($all_news as $news): $i++;
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <span class="badge badge-<?= htmlspecialchars($news->active == '1' ? 'success' : 'danger', ENT_QUOTES, 'UTF-8'); ?>"><?= htmlspecialchars($news->active == '1' ? 'Active' : 'Inactive', ENT_QUOTES, 'UTF-8'); ?></span>
                                                </td>
                                                <td>
                                                    <a href="<?= site_url('panel/news/edit/' . $news->newsroom . '/' . $news->language) ?>"
                                                       title='Edit' class="btn-sm btn-primary"><i
                                                            class="fas fa-user-edit"></i></a>
                                                    <a href="#" class="btn-sm btn-danger trigger_alert_modal" data-title="Confirm" data-desc="Are you sure want to delete this? <p class='text-danger'><i>(If you confirm, the item will be permanently removed from the server. The process cannot be reverted.)</i></p>" data-redirect="<?= site_url('panel/news/delete/' . $news->id) ?>" title='Delete'><i class="fas fa-trash-alt"></i></a>
