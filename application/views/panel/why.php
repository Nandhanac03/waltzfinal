                ...
                <div class="col-12">
                    ...
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">Why Work with Us</h4>
                            <?php if ($controller_config['disable_blog_add'] != TRUE) : ?>
                                <a href="<?= site_url('panel/why/add') ?>" class="btn btn-sm btn-info float-right" title="Add"><i class="fas fa-user-plus"></i> Add</a>
                            <?php endif; ?>
                        </div>
                        <!-- /.card-header -->
                        <div class="card-body">
                            <form role="form" method="post" action="<?= site_url('panel/why/all') ?>">
                            ...
                            <table class="table table-bordered table-striped data-table-search-off">
                                ...
                                <tbody>
                                    <?php
                                    if ($why_work_with_us) : $i = 0;
                                        foreach ($why_work_with_us as $each_job) : $i++;
                                    ?>
                                            <tr>
                                                <td><?= $i ?></td>
                                                <td><?= htmlspecialchars($each_job->title, ENT_QUOTES, 'UTF-8'); ?></td>
                                                <td><?= htmlspecialchars($each_job->short_desc, ENT_QUOTES, 'UTF-8'); ?></td>
                                                ...
                                                <td>
                                                    <a href="<?= site_url('panel/why/edit/' . $each_job->id) ?>" title='Edit' class="btn-sm btn-primary"><i class="fas fa-user-edit"></i></a>
                                                </td>
                                            </tr>
                                    <?php
                                        endforeach;
